...
## DMARC（Domain-based Message Authentication）机制与绕过

### 机制
DMARC 建立在 SPF 和 DKIM 之上，告诉接收方：如果 SPF 和 DKIM 都失败了，应该如何处理这封邮件？

DMARC DNS 记录格式：_dmarc.company.com. IN TXT "v=DMARC1; p=reject; sp=reject; adkim=s; aspf=s; pct=100; rua=mailto:dmarc@company.com"

对齐模式（Alignment Mode）详解：DMARC 要求 SPF 或 DKIM 的"对齐"检查通过才算有效：严格模式（adkim=s / aspf=s）:
...
  From: user@company.com
  DKIM d=: mail.company.com → ✅ 对齐通过（宽松模式）3.2 DMARC 策略的实际执行差异p=none（仅监控）：v=DMARC1; p=none; rua=mailto:dmarc@company.com邮件仍然会被投递，不会拦截。只是向 rua= 指定的地址发送汇总报告。

现状：约 60% 配置了 DMARC 的企业使用 p=none，原因是不敢贸然开启 reject 怕误拦合法邮件。p=quarantine（隔离）：邮件会被放入垃圾邮件文件夹，用户可以手动找回。p=reject（拒绝）：邮件在 SMTP 层面被拒绝（返回 550 5.7.1 错误），不会进入收件箱。
